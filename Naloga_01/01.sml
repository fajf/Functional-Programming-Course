...
fun len (xs : int list) : int =
    let
        fun len (xs, dolzina) =
            if (null xs) then dolzina
            else len (tl xs, dolzina+1)
    in
        len (xs, 0)
    end;
...
fun delete (xs : int list, x : int) : int list = 
    let
        fun delete(xs : int list, x : int, new : int list) = 
            if null xs then new
            else
                if  hd xs = x then delete(tl xs, x, new)
                else delete(tl xs, x, new@[(hd xs)])
    in
        delete(xs, x, [])
    end;
...
fun reverse (xs : int list) : int list = 
    let
        fun reverse(xs : int list, new : int list) =
            if null xs then new
            else reverse(tl xs, (hd xs)::new)
    in
        reverse(xs, [])
    end;
...
fun palindrome (xs : int list) : bool =
    if xs = reverse(xs) then true else false;